 ...
 * given index.
 */
#define da_slice(Type, arr, start, slice_size)\
({\
    Type* new_arr = da_init(Type);\
 ...
 */
#define da_slice_sv(Type, arr, start, end)\
    (da_slice(Type, arr, start, end - start + 1))


/**
 * Creates an array of values derived from the given function
 */
#define da_map(Type, arr, func)\
({\
    Type* new_arr = da_init(Type);\
 ...
 */
#define da_filter(Type, arr, func)\
({\
    Type* new_arr = da_init(Type);\
    for (int i = 0; i < arr->size; i++) {\
